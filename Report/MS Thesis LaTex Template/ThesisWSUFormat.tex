...
%title page
\centering
\large{PRIVACY-PRESERVING DISTRIBUTED DEEP LEARNING}\\
[8\baselineskip]
\doublespace{
A Thesis by\\
Aseem Prashar\\
...
I would like to thank my adviser, Dr. Sergio A. Salinas Monroy, for his thoughtful input, guidance and support in all stages of this project. I also extend my gratitude to members
of my committee, Dr. Ajita Rattani, Dr. Remi Chou and Dr. Akmal Mirsadikov for their valuable time and consideration. 
...
\doublespacing{
\begin{align*}
&GAN &&\text{Generative Adversarial Network}\\
&SMC &&\text{Secure Multi-party Computation}\\
&MLP &&\text{Multilayer Perceptron}\\
&GD &&\text{Gradient Descent}\\
&DNN &&\text{Deep Neural Network}\\
&SGD &&\text{Stochastic Gradient Descent}\\
&PS &&\text{Parameter Server}
...
&M &&\text{ Mini batch size used for stochastic gradient descent}\\
&\theta_u^{i} &&\text{ Fraction of parameters selected for upload for $i$th user}\\
&\theta_d^{i} &&\text{ Fraction of parameters selected for download for $i$th user}\\
&W_k &&\text{ Weight matrix for layer K in neural network}\\
&w_i &&\text{ Flattened vector of all parameters in neural network for $i$th user} \\
&\Delta w_i &&\text{ Vector of changes in all local parameters due to gradient descent}\\
&w^{(global)}&&\text{Flattened parameter vector for server}\\
&E &&\text{ Error function computed over expected value of the objective function} \\
&\alpha &&\text{ Learning rate of the stochastic gradient descent algorithm}\\
&S_i &&\text{ Set of indices selected from flattened vector of parameters for $i$th user}\\
&u_0 &&\text{Reference user}
%make sure to have no space on your last line
\end{align*}
}
\end{center}
...
a parameter server (PS), as shown in Figure \ref{fig:HighLevelOld}. User $u_0$ aims to train a local deep neural network using its own training data set
...
\begin{figure}[H]
  \centering
    ...
    \caption[Privacy preserving architecture.]{\label{fig:HighLevelOld} An architecture for privacy-preserving distributed learning.}
  \end{figure}
\addtocontents{lof}{\vspace{\normalbaselineskip}}
...
\cite{shokri2015privacy}, we focus on protecting the privacy of a single user, called the reference users as described in
Section \ref{sec:systemModel}. Our main approach to protect the reference user's privacy is to prohibit the reference user
from uploading its parameters to the  parameter server, as shown in Figure \ref{fig:HighLevel}.
