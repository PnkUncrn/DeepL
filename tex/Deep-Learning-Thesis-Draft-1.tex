...
%---------------------------------------------------------------------------------
This is where you can talk about what are DNNs and training....

The cost function is given by the average difference between the value predicted by the network and the actual value during the
training phase.
%In the context of neural networks, we use Gradient Descent algorithm to optimize the cost function. 

Cost function is an indicator of how accurate the network is where a network with lower resulting cost is behaving as intended.
Standard gradient descent algorithm computes gradient over the whole dataset. This implies we would process all the samples in the
training set to do a single update for a parameter in a particular iteration. For larger datasets, this is incredibly time consuming.
...
Although GD is effective at finding the parameters of DNNs, since it employs all the samples the data set it is time
consuming. To overcome this challenge, Stochastic Gradient Descent(SGD) hass been proposed. SGD selects a random sample for calculating
the gradient at each iteration. Since we are using one sample at a time to calculate the gradient, the path to local the minimum is
noisier (WHAT DO YOU MEAN BY NOISIER?) than GD. However, the stochastic GD significantly reduces the training time of the network
DO WE ACHIVE THE SAME RESULTS? IF NOT WHAT IS THE TRADE-OFF?


The most popular gradient descent algorithm, however, is mini-batch Gradient Descent. This algorithm uses more than 1 sample for
calculating gradient descent at each iteration and then the model is updated based on smaller groups of samples. If the minibatch is of
size $M$, then $1 < M < k$, where $k$ is the total size of the training dataset.  Mini-batch SGD converges in fewer iterations than GD
while being less noisier (AGAIN, WHAT IS THE NOISE HERE?) than SGD.
