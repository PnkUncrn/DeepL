...
In this section we describe an inventive approach to ensuring privacy and preventing GAN based attack proposed in /ref{}. Our proposal works by ensuring Privacy to the Reference User by limiting its exposure to other collaborators. Out method facilitates parameter sharing for other collaborators as well. However, we structure this algorithm in which a primary participant, the reference User benefits the most and is afforded the highest level of protection against GAN-based attacks. We envision this architecture possibly materializing in a manner that involves the Reference user compensating other collaborators for the parameters and abstract data to train its model. Since the architecture inherently obscures the absolute data, we are also able to preserves privacy for paid collaborators involved.
 
We define the following terms, $\theta_u$
$R$
Our Algorithm works as follows:
\begin {enumerate}
\item A pool of participants is with a sizeable dataset is created
\item Out of the pool we randomly select a subsection of participants to participate in this period. We call it a round denoted by $r$.
\item In each round, $r_i$, the selected participant, $u_i$ gets an atomic lock on the Parameter Server.
\item During its interaction with PS, the participant will perform the following steps:
\begin {enumerate}
  \item Train independantly on its local dataset for one epoch, to obtain the gradients in its parameters, $\Delta w$ 
  \item  At the end of the epoch, the participant will select the $\theta_u$ highest gradients, $\Delta w$.  $u_i$ uploads these values to the PS.
  \end {enumerate}
...
\item We repeat these steps for every participant selected in the round. At the end of the round, the Reference User downloads $\Theta_d$ percentage of parameters from the Server.
